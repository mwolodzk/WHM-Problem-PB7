...
class analizatorWynikow:
    ...
    * porównuje różne algorytmy schładzania,
    ...
    def __init__(self, parametryDoswiadczenia):
        ...
        self.dane = []              # lista słowników zawierających dane z kolejnego doświadczenia
...
#        "Zapamiętuje rozwiązanie i koszty uzyskania rozwiązania z algorytmu symulowanego wyżarzania."
    ...
    def _porownajSchematySchladzania(self, daneZDoswiadczen):
        ...
        """
        # uporządkowanie danych
        schematy = [0,0,0]; fEmin = [0,0,0]; iteracje = [0,0,0]; ktory = None;
        for daneDoswiadczenia in daneZDoswiadczen:
            schemat = daneDoswiadczenia['schemat']
            if schemat == "Boltzmann" or schemat == 'Logarytmiczny': ktory = 0
            elif schemat == "Cauchy'ego" or schemat == "Liniowy": ktory = 1
            elif schemat == 'Geometryczny': ktory = 2
            else: ktory = -1
            if ktory == -1: raise ValueError, "Podana niepoprawna nazwa schematu schładzania – " + str(schemat)
            schematy[ktory] += 1
            fEmin[ktory] += daneDoswiadczenia['fEmin']
            iteracje[ktory] += daneDoswiadczenia['iteracje']
        
        # obliczenie wartości miarodajnych
        fEminAvg = [0,0,0]; iteracjeAvg = [0,0,0]
        for i in range(3):
            fEminAvg[i] = fEmin[i] / schematy[i] if schematy[i] > 0 else 0
            iteracjeAvg[i] = iteracje[i] / schematy[i] if schematy[i] > 0 else 0
        ...
        * tworzy wykresy złożoności obliczeniowej algorytmu w zależności od jego parametrów.
        ...
        def __init__(self, dane, wykresy=None):
            ...
            if self.wykresy == None:
                plt.figure(); self._rysujWykresZlozonosciObliczeniowej()
                plt.figure(); self._rysujWykresT()
                # plt.figure(); _rysujPorownanieAlgorytmow()
                plt.figure(); self._rysujPorownanieSchematowSchladzania()
            elif "O(n)" in self.wykresy:
                plt.figure(); self._rysujWykresZlozonosciObliczeniowej()
            elif "E(T)" in self.wykresy:
                plt.figure(); self._rysujWykresT()
            elif "A(J,NJ)" in self.wykresy:
                plt.figure(); self._rysujPorownanieAlgorytmow()
            elif "Sch" in self.wykresy:
                plt.figure(); self._rysujPorownanieSchematowSchladzania()
            elif "Rozw" in self.wykresy:
                plt.figure(); self._rysujDochodzenieDoRozwiazania()
                
            plt.show()
            
        def _normalizujDane(self):
            "Normalizuje dane do rysowania do formatu przyjmowanego przez PyPlot"
            for klucz in self.dane:
                if klucz == 'zlozonoscCzasowa'
        ...
        def _rysujWykresZlozonosciObliczeniowej(self):
            ...
            nt = np.asarray(zlozonoscCzasowa[1])
            Ot = np.asarray(zlozonoscCzasowa[0])
            
            nm = np.asarray(zlozonoscPamieciowa[1])
            Om = np.asarray(zlozonoscPamieciowa[0])
